...
SELECT 
    ...
    CASE 
        ...
        WHEN p.qtde_produto >= 7 THEN 'NORMAL'
...
-- Consultar a view:
-- SELECT * FROM vw_dashboard_estoque WHERE status_estoque IN ('CRÍTICO', 'ALERTA');


-- VIEW 2: Relatório Consolidado de Pedidos por Produto
-- Objetivo: Análise de padrão de reposição por produto
CREATE OR REPLACE VIEW vw_relatorio_pedidos_produto AS
SELECT 
    p.cod_produto,
    p.nome_produto,
    c.nome_categoria,
...
-- Consultar a view:
-- SELECT * FROM vw_relatorio_pedidos_produto WHERE total_pedidos >= 2;